...
function gameStart() {
    if(!gameStarted) {
        this.platformHorizontal = new Platform(100, 20, 250, 370, "red");
        if(verticalPlatformActive) {
            this.platformVertical = new Platform(20, 100, 10, 150, "red");
        }
        this.balls.push(new GameBall(200 + getRandomInt(200), 200, 10, "blue", 2, getRandomInt(2)));
        this.balls.push(new GameBall(200 + getRandomInt(200), 200, 10, "blue", 3, getRandomInt(2)));
        myGameArea.start();
...
function checkPlatformClipping() {
    ...
    if((this.platformHorizontal.x + this.platformHorizontal.width >= 600 && this.platformHorizontal.speedX>0) ||
    ...
    if(verticalPlatformActive)
        {if((this.platformVertical.y + this.platformVertical.height >= 400 && this.platformVertical.speedY>0) ||
...
function showScore() {
    ...
    let ctx = canvas.getContext("2d");
    ctx.font = "10px Arial";
    ctx.fillText("Score: " + score, 500, 15);
}
...
function showTime() {
    ...
    let ctx = canvas.getContext("2d");
    ctx.font = "10px Arial";
    ctx.fillText("Time: " + showTimeCurrent, 500, 30);
}
